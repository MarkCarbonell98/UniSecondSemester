...
int * get_waiting_times(int * arr, int length) {
    int * waiting_times = malloc(sizeof(int) * MAX_SIZE);
    int counter = 0;
    for(int i = 0; i < length; i++) {
        waiting_times[i] = counter + arr[i];
        counter += arr[i];
    }
    print_array(waiting_times, length);
...
int main(int argc, char const *argv[])
{
    ...
    int * waiting_times = get_waiting_times(third_process_times, process_amount);
    free(waiting_times);
